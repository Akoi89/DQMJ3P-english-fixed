...
Check the `.cia` sizes before you decrypt anything, since the decrypt takes a while and a wrong file can only fail at the end of it. The base is the one people get wrong. If yours isn't 1,591,612,416 bytes it's a repack, a cartridge dump or a build with the update already merged in, and no patch can bridge that. There's only one correct Japanese base, and the numbers above are what it is. In most dump filenames it's listed as `ドラゴンクエストモンスターズ - Joker 3 PROFESSIONAL`, product code CTR-P-BDQJ. The game's own banner writes that as `ドラゴンクエストモンスターズ` on one line and `Joker 3 PROFESSIONAL` on the next; the hyphen is just how the dump tools flatten the line break.
